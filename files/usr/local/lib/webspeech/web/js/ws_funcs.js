// JS support functions for Bhojpur Speeh processing engine.
...
function init_state() {
  ...
  function wait_for_server() {
    if (ws_wait) {
      showMsg("waiting for Bhojpur Speech processing server ...",1000);
  ...
  */
  ws_isPause = false;
  ws_update_play_time_id = null;
  function update_play_time(force=false) {
    if (!ws_isPause || force) {
      // update elapsed time and display
      ws_state.player.time[0] += 1;
      h = ~~(ws_state.player.time[0] / 3600);
      m = ~~((ws_state.player.time[0]-h*3600) / 60);
      s = Math.round(ws_state.player.time[0] -h*3600 - m*60);
      if (h>0) {
        elapsed = formatTime(h)+':'+formatTime(m)+':'+formatTime(s);
      } else {
        elapsed = formatTime(m)+':'+formatTime(s);
      }
      $("#ws_time_cur").text(elapsed);
      elapsed_pc = 100*ws_state.player.time[0]/ws_state.player.time[1];
      $("#ws_time_range").val(elapsed_pc);
      $("#ws_time_range").css("background-size",elapsed_pc+"% 100%");
    }
  }
  ...
  */
  ...
  function openTab(tabId,data) {
    ...
    if (tabId === ws_state.webgui.tabid) {     // nothing to do
      return;
    } else {
      ws_state.webgui.tabid = tabId;
    }
    ...
    // update new state
    if (data !== 'sys') {
      $.post('/api/update_state',JSON.stringify(ws_state));
    }
  };
  ...
  */
  ...
  function on_open_tab_play(internal=false) {
    // record-button only relevant for Bhojpur Speech
    if (ws_state.mode == "player") {
      $('#ws_rec_btn').hide();
      $('#ws_time').show();
    } else {
      $('#ws_rec_btn').show();
  ...
  */
  ...
  function handle_event_state(data) {
    ...
    if (data.webgui) {
      if (data.webgui.tabid) {
        update = ws_state.webgui.tabid !== data.webgui.tabid;
      }
    }
    if (data.mode) {
      if (!ws_state.mode) {
        ws_wait = false;                             // stop waiting message
        $(".tablink").removeClass("menu_disabled");  // enable everything
      }
      ws_state.mode = data.mode;
    }
    if (data.player && data.player.time) {
      ws_state.player.time[1] = data.player.time[1];
      $("#ws_time_tot").text(data.player.time[2]);
    }
  ...
  function handle_event_play(file) {
    ...
    clearInterval(ws_update_play_time_id);
    ws_isPause = false;
    ws_update_play_time_id = setInterval(update_play_time,1000);
  
    // enable pause-button
    $('#ws_pause_btn').removeClass('far').addClass('fas').prop("disabled", false);
  
    // update state
    if (ws_state.mode === 'player') {
      ws_state.player.last_file = file;
      // update highlight
      if (ws_state.player.last_index > -1) {
        $('#f_'+ws_state.player.last_index).removeClass('file_item_selected');
      }
      ws_state.player.last_index = ws_file2index[file];
      $('#f_'+ws_state.player.last_index).addClass('file_item_selected');
      cover_url = '/api/player_get_cover?dir='+encodeURIComponent(
            ws_state.player.last_dir);
      $('#ws_play_logo').attr('src',cover_url);
    }
  }
  
  function handle_event_pause(data) {
    ws_isPause = true;
    $('#ws_pause_btn').removeClass('fas').addClass('far');
  }
  
  function handle_event_eof(data) {
    if (ws_update_play_time_id) {
      clearInterval(ws_update_play_time_id);
      ws_update_play_time_id = null;
    }
    $('#ws_infos').empty();
  ...
  function handle_event_bhojpur_play_channel(data) {
  ...
  function handle_event_dir_select(data) {
    if (ws_state.player.last_dir !== data) {
  ...
  function handle_event_file_info(data) {
    if (!ws_update_play_time_id) {
      ws_state.player.time[0] = 0;
      $("#ws_time_cur").text("00:00");
    }
    ws_state.player.time[1] = data.total;
    $("#ws_time_tot").text(data.total_pretty);
  }
  
  function handle_event_sample(data) {
    ws_isPause = data.pause;
    ws_state.player.time[0] = ~~(data.elapsed*ws_state.player.time[1]);
    if (ws_isPause) {
      ws_state.player.time[0] -= 1;
  ...
  */
  ...
  function update_channel_info(channel) {
    $('#ws_infos').empty();
    if (channel.logo) {
      $('#ws_play_logo').attr('src',channel.logo);
      $('#ws_play_name').empty();
    } else {
      $('#ws_play_logo').attr('src','/images/default.png');
      $('#ws_play_name').html(channel.name);
    }
  }
  ...
  function update_player_list(dirInfo) {
    ...
    $.each(dirInfo.files,function(index,file) {
        ...
        if (file == dirInfo.cur_file) {
          ...
          ws_state.player.last_index = index;
          ws_state.player.last_file  = file;
        }
      });
  }
  ...
  */
  ...
  function bhojpur_play_channel(data) {
    ...
    $.getJSON('/api/bhojpur_play_channel',data,
      function(channel) {
  ...
  */
  ...
  function player_select_dir(data) {
    ...
    var last_dir = ws_state.player.last_dir;
    if (data.dir != last_dir) {
      ...
      ws_state.player.last_dir = last_dir;
    } else {
      return;
    }
  ...
  */
  ...
  function vol_mute_toggle() {
    ...
    $('#ws_mute_btn').toggleClass('fa-volume-mute').toggleClass('fa-volume-off');
